...
<div class="container my-3">
    ...
    <table class="table">
        ...
        <tr class="text-center">
            ...
            <td>{ question.user ? question.user.username : "Admin" }</td>
            <td>
                {#if moment().diff(moment(question.create_date), 'days') >= 7}
            ...
            <td>
                <span class="text-danger small mx-2">{question.voter.length}</span>
            </td>
        </tr>
        {/each}
        </tbody>
    ...
    <li class="page-item { $page === 0 && 'disabled' }">
        ...
        <!-- 마지막 페이지로 이동 -->
        <li class="page-item { $page === total_page - 1 && 'disabled' }">
            <button class="page-link" on:click={() => $page = total_page - 1}>마지막({total_page})</button>
        </li>
    </ul>
    <!-- 페이징처리 끝 -->
</div>